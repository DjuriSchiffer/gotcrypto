...
export const useStorage = () => {
	...
	useEffect(() => {
		const initialize = async () => {
			...
			if (user && !isAnonymous) {
				...
				const currencyQuoteFromStorage =
					await localforage.getItem<keyof CurrencyQuote>(CURRENCY_QUOTE);
	...
	const updateCurrency = useCallback(
		async (updatedAsset: SelectedAsset) => {
			...
			const assetExists = selectedCurrencies.some((asset) => asset.cmc_id === updatedAsset.cmc_id);

			if (assetExists) {
				updatedAssets = selectedCurrencies.map((asset) =>
					asset.cmc_id === updatedAsset.cmc_id ? updatedAsset : asset
				);
			} else {
		...
		[selectedCurrencies, setSelectedCurrenciesState, user, isAnonymous, setLocalForage]
	);
